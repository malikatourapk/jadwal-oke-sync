...
export const useSupabasePOS = () => {
  ...
  useEffect(() => {
    ...
    // Set up real-time subscriptions with reload on any change
    ...
    const receiptsSubscription = supabase
      .channel('receipts_realtime_channel')
      .on('postgres_changes', 
    ...
    const receiptItemsSubscription = supabase
      .channel('receipt_items_realtime_channel')
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'receipt_items' }, 
        () => {
          console.log('Receipt items changed, reloading...');
          loadReceipts();
        }
      )
      .subscribe();
    ...
    return () => {
      ...
      supabase.removeChannel(receiptItemsSubscription);
    };
  }, [user]);
