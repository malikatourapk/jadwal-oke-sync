...
export const StoreSettings = () => {
  ...
  useEffect(() => {
    if (currentStore) {
      setFormData({
        ...
        whatsapp_report_number: (currentStore as any).whatsapp_report_number || '',
        admin_password: currentStore.admin_password || '12345',
        settings_password: currentStore.settings_password || '12345',
      });
    }
  }, [currentStore]);
  ...
  const handleSave = async () => {
    ...
    const payload: Record<string, any> = {
      ...
      whatsapp_report_number: formData.whatsapp_report_number,
          ...
          <div className="grid gap-4 sm:gap-6">
            ...
            <Card className="border-border">
              ...
              <CardContent className="space-y-3 sm:space-y-4">
                ...
                <div>
                  <Label htmlFor="whatsapp_report_number" className="text-xs sm:text-sm">Nomor WhatsApp (untuk Laporan PDF)</Label>
                  <Input
                    id="whatsapp_report_number"
                    value={formData.whatsapp_report_number}
                    onChange={(e) => handleInputChange('whatsapp_report_number', e.target.value)}
                    placeholder="08xx xxxx xxxx"
                    className="h-9 sm:h-10 text-sm"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    Nomor WhatsApp untuk mengirim laporan penjualan (bisa berbeda dengan nomor daftar belanja)
                  </p>
                </div>
